...
#Calculate deratings for NON-NUCLEAR THERMAL POWER PLANTS ONLY. Set derates to 0 for other plants.
...
def calculatePlantCapacityDerates(genFleet, newTechsCE, demand, weatherYears, climateScenario, compressedGens, tz):
    ...
    #Load weather
    metVars = importTGWMetVars(weatherYears,climateScenario)

    #Derate existing units
    # capDerates = derateExistingUnits(genFleet,capDerates,metVars,compressedGens)
    capDerates = derateExistingUnits(genFleet,metVars,compressedGens)
    ...
    capDeratesTechs,newTechsCE = calculateNewTechDerates(newTechsCE,metVars)

    # capDerates = pd.DataFrame(0,index=demand.index,columns=genFleet['GAMS Symbol'])
    # capDeratesTechs = pd.DataFrame(0,index=demand.index,columns=newTechsCE['GAMS Symbol'])
        
    #Shift tzs from UTC to ET
    capDerates = shiftToTz(capDerates,weatherYears,tz)
    capDeratesTechs = shiftToTz(capDeratesTechs,weatherYears,tz)

    #Fill in missing indices in derates (last few hours of year due to timeshift)
    missing = demand.index.difference(capDerates.index)
    capDerates = pd.concat([capDerates,pd.DataFrame(0,index=missing,columns=capDerates.columns)])
    capDeratesTechs = pd.concat([capDeratesTechs,pd.DataFrame(0,index=missing,columns=capDeratesTechs.columns)])
    
    return capDerates,capDeratesTechs,newTechsCE

def shiftToTz(df,weatherYears,tz,tzOffsetDict = {'PST':-7,'CST': -6,'EST': -5}):
    df.index = df.index.shift(tzOffsetDict[tz],freq='H') #shift for tz
    df = df[df.index.year.isin(weatherYears)] #filter to year of interest
    # df = df.append([df.iloc[-1]]*abs(tzOffsetDict[tz]),ignore_index=True) #replace dropped hours at end of year so have full year
    return df
...
def calculateNewTechDerates(newTechsCE,metVars):
    ...
    #Fill in any missing generators with 0s
    missed = pd.Index(newTechsCE['GAMS Symbol']).difference(deratesTechs.columns)
    deratesTechs[missed] = 0

    return deratesTechs,newTechsCE
...
def derateExistingUnits(genFleet,metVars,compressedGens):
    #Set recirculating and dry cooling design assumptions
    rcDesign,dcDesign = setCoolingDesigns()
    ...
    #Fill in any missing generators with 0s
    missed = pd.Index(genFleet['GAMS Symbol']).difference(derates.columns)
    derates[missed] = 0
...
def getClosestCellCoordsWithMet(metVars,lat,lon,tVar='TREFHT'):
    ...
    while np.isnan(metVars.isel(time=-1).sel({'lat':lat,'lon':lon},method='nearest').compute()[tVar]):
