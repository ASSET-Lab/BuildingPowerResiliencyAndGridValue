...
def defineTransmissionRegions(region,balAuths):
    transRegions = dict()
...
################################################################################
def setupTransmissionAndZones(genFleetFull,transRegions,region,balAuths):
    #Read regions and map to zones
    pRegionShapes = loadRegions(transRegions)
    #Add offshore wind locations to p-regions
    pRegionShapes = addOffshoreWindRegionsToPRegions(pRegionShapes)
    #Assign generators to zones (through p-regions)
    genFleet = assignGensToPRegions(genFleetFull.copy(),pRegionShapes)
    if region == 'ERCOT': genFleet = pd.concat([genERCOTRegionReassign(genFleetFull.copy()),genFleet])
    checkZones(genFleet,transRegions) 
    #Get transmission constraints between zones
    limits,costs,dists = getTransmissionData(region,transRegions,pRegionShapes)
    return genFleet,transRegions,limits,dists,costs,pRegionShapes

#Following function uses unary_union to combine 2 shapefiles into a single shape. 
#Inputs: p regions shapefile
#Outputs: a single gpd geodataframe with 1 row per p-region, where each row has combined offshore wind region + p region
from shapely.ops import unary_union
def addOffshoreWindRegionsToPRegions(pRegionShapes,shapeDir=os.path.join('Data','RenewableShapefilesNYC')):
    print('Adding offshore wind regions to p127 and p128.')
    offShape = gpd.read_file(os.path.join(shapeDir,'offshore_wind.shp'))
    offShape['region']=['p127','p128'] #assign the 2 wind zones each to a p-region

    combodRegions = list()
    for r in ['p127','p128']:
        shape1,shape2 = pRegionShapes.loc[pRegionShapes['region']==r]['geometry'].values[0],offShape.loc[offShape['region']==r]['geometry'].values[0]
        combod = unary_union([shape1,shape2])
        combod = gpd.GeoDataFrame({'geometry': [combod],'region':[r]})
        combodRegions.append(combod)

    combodRegions = pd.concat(combodRegions)    
    combodRegions.reset_index(inplace=True,drop=True)
    # combodRegions.plot()
    # combodRegions.loc[combodRegions['region']=='p127'].plot()
    # combodRegions.loc[combodRegions['region']=='p128'].plot()
    # print(combodRegions)
    return combodRegions
...
##########################
def getTransmissionData(region,transRegions,pRegionShapes):
    #Import raw transmission data from REEDS
    limits,costs,dists = importTransmissionData()
    #Filter data to p-regions of interest and, if using larger regions, combine p-region data
    limits,costs,dists = filterOrCombineTransmissionData(region,limits,costs,dists,transRegions,pRegionShapes)
    dists,limits,costs = expandTransmissionData(region,limits,costs,dists)
...
def filterOrCombineTransmissionData(region,limits,costs,dists,transRegions,pRegionShapes):
...
def expandTransmissionData(region,limits,costs,dists):
    #Reverse source-sink limits
    limits = addReversedLines(limits)
    if region == 'EI' or region == 'WECC': #reverse dist & cost limits
