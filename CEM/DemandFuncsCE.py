...
def getHoursForCE(demand,netDemand,windGen,solarGen,daysPerRepBlock,daysPerPeak,
    ...
    blockRepNetDemand,blockAllNetDemand = getRepBlockHoursByNLDC(dfNet,dfNoSpecial,daysPerRepBlock,numBlocks) #**need this if don't use jill's days
    blockWeights = calculateBlockWeights(dfTotal,blockRepNetDemand,blockAllNetDemand) #**need this if don't use jill's days
...
def getHeatWaveDays(df,heatWaveDts,dfNoSpecial):
    dfHeatWave = df[heatWaveDts]
    dfNoSpecial = dfNoSpecial[~dfNoSpecial.isin(peakDays)].dropna()
    return dfNoSpecial,dfHeatWave
