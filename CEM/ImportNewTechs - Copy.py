...
def getNewTechs(regElig,regCostFrac,currYear,stoInCE,seasStoInCE,fuelPrices,yearIncDACS,yearIncHydrogen,yearIncCCS,yearIncNuclear,
                transRegions,contFlexInelig,weatherYears,climateScenario,genFleet,ira,lbToShortTon,onlyNSPSUnits=True,
...
def repeatNonRETechOptionsForEachRegion(newTechsCE,transRegions):
    ...
    for r in transRegions:
        regionTechs = newTechsNotRE.loc[~newTechsNotRE['PlantType'].isin(['Onshore Wind','Offshore Wind','Solar PV'])].copy()
...
def repeatNonRETechOptionsForEachCell(newTechsCE,transRegions,genFleet,climateScenario,weatherYears,existingPTs=['Coal Steam','Combined Cycle','Nuclear']):
    #Separate RE from non-RE
    newTechsRE = newTechsCE.loc[newTechsCE['PlantType'].isin(['Onshore Wind','Offshore Wind','Solar PV'])].copy()
    newTechsNotRE = newTechsCE.loc[~newTechsCE.index.isin(newTechsRE.index)].copy()
    #Import temperatures
    # print(climateScenario)
    if 'rcp' not in climateScenario: metVars = importCESMMetVars(weatherYears,climateScenario)
    else: metVars = importTGWMetVars(weatherYears,climateScenario)
